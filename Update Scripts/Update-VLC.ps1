...
$Product="VLC"
$EvergreenName="VideolanVLCPlayer"
...
$DesktopShortcutExists = Test-Path "$($env:Public)\Desktop\VLC Media player.lnk"
...
If (!(Test-Path -Path $InstallerPath -PathType Container)) {
	$null = New-Item -ItemType Directory -Force -Path $InstallerPath
}
...
If (!($ProdCheck)) { 
    # VLC has uninstalled, need to re-run installer and collect version details again
    Start-Process "msiexec.exe" -ArgumentList  "/i $Installer /qn /l*v $logfile" -NoNewWindow -Wait
	Start-Sleep -Seconds 30
...
If (($DesktopShortcutExists -eq $false) -AND ((Test-Path "$($env:Public)\Desktop\VLC Media Player.lnk") -eq $True)) {
    Write-Host "Removing surplus desktop shortcut"
