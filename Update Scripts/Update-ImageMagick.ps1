...
$Product="ImageMagick"
$EvergreenName="ImageMagickStudioImageMagick"
...
If (Test-Path $Installer -PathType Leaf) {
     ...
     #ImageMagick requires uninstall of the existing version first
	 $Uninstaller = $Prod.UninstallString
	 $UninstallerArgs = "/SILENT"
     Start-Process $Uninstaller -ArgumentList $UninstallerArgs -NoNewWindow -Wait
