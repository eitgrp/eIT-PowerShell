...
$Prod=$Apps | Where-Object { $_.DisplayName -match "Notepad\+\+" }
...
If ($Prod) {
    ...
    $evergreenapp = get-evergreenapp -Name $EvergreenName | Where-Object { $_.Architecture -eq $targetarchitecture -and $_.InstallerType -eq $EvergreenInstallerType } | Select-Object -First 1
...
$Apps+=Get-ItemProperty "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*" # 32 Bit
$Apps+=Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*"             # 64 Bit
$ProdCheck=$Apps | Where-Object { $_.DisplayName -match "Notepad\+\+" }
