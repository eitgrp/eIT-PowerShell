#!ps
#timeout=600000
## Install / Update Paint.NET
$Product="Paint.NET"
$EvergreenName="PaintDotNetOfflineInstaller"
$EvergreenType="msi"  # msi or exe
...
If ($Prod) {
    ...
    $evergreenapp = get-evergreenapp -Name $EvergreenName | Where-Object { $_.Architecture -eq 'x64' -AND $_.URI -match '.winmsi.' } | Select-Object -First 1
...
While ($true -and ($Downloads -lt 10)) {
    ...
    If (((Get-Item $Installerpackage).Length/1024000) -gt $MinimumSizeMB) { 
        ...
        $evergreenapp = get-evergreenapp -Name $EvergreenName | Select-Object -First 1
        $DownloadPackage=$evergreenapp.URI
        Write-Host "        FAILED"
    }
}
...
$ExpandedPackage=Expand-Archive $Installerpackage -DestinationPath $InstallerPath -Force
$Installer=Get-ChildItem -LiteralPath $InstallerPath | Where-Object { $_.Name.Substring(($_.Name.Length)-4) -eq '.msi' }
...
If ($Installer) {
     ...
     Start-Process "msiexec.exe" -ArgumentList  "/i $($Installer.Directory)\$($Installer.Name) /qn /l*v $logfile" -NoNewWindow -Wait
     If ($rds) {
