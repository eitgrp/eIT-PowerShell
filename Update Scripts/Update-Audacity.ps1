#!ps
#timeout=600000
## Install / Update Bitwarden
$Product="Audacity"
$EvergreenName="Audacity"
...
$DesktopShortcutExists = Test-Path "$($env:Public)\Desktop\Audacity.lnk"


#Get the current architecture
If ($Prod) {
    # Get the current release for the selected architecture
    $evergreenapp = get-evergreenapp -Name $EvergreenName | Where-Object { $_.Type -eq $EvergreenType } | Select-Object -First 1
...
If (Test-Path $Installer -PathType Leaf) {
     ...
     Start-Process $Installer -ArgumentList "/VERYSILENT /LOG=$logfile" -NoNewWindow -Wait
     If ($rds) {
...
If (($DesktopShortcutExists -eq $false) -AND ((Test-Path "$($env:Public)\Desktop\Audacity.lnk") -eq $True)) {
    Remove-Item "$($env:Public)\Desktop\Audacity.lnk"
}
